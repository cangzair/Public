...
function index()
	...
	local page
	page = entry({"admin", "vpn", "shadowsocksr"}, alias("admin", "vpn", "shadowsocksr", "client"), _("SSR Plus+"), 10)
	page.dependent = true
	...
	entry({"admin", "vpn", "shadowsocksr", "ping"}, call("act_ping"))
...
function act_reset()
	...
	luci.http.redirect(luci.dispatcher.build_url("admin", "vpn", "shadowsocksr"))
end
